...
function formatLocationMode(value) {
  if (value === "gps") return "Live vehicle GPS";
  if (value === "fixed") return "Fixed start point";
  if (value === "none") return "None";
  return "Unknown";
}

export default function MissionPanelCarInline({
  runtimeState = null,
  ...
  const [gpsMode, setGpsMode] = useState("required");
  ...
  const activeLocationMode =
    activeProfile?.location_mode || locationMode || "gps";
  ...
  async function handleStartClick() {
    ...
    setErrorText(result?.error || "Drive session could not be started.");
  }
      ...
      {!missionRunning ? (
        <>
          <div className="mt-5 grid grid-cols-1 gap-4">
            <label className="form-control w-full">
              <SectionLabel>Route session name</SectionLabel>
              <input
                className="input input-bordered w-full rounded-xl"
                type="text"
                maxLength={120}
            ...
            <div className="overflow-hidden rounded-2xl border border-base-300 bg-base-100">
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-base-content/55">
                    Sampling and live telemetry behavior
                  </div>
                </div>
                <FiChevronDown
              ...
              {advancedOpen && (
                <div className="grid grid-cols-1 gap-4 border-t border-base-300 px-4 py-4">
                  <NumberField
            ...
            <div>
              <SectionLabel>Route origin</SectionLabel>
            ...
            <div className="rounded-2xl border border-info/30 bg-info/10 px-4 py-3">
              <div className="flex items-center gap-2 text-sm font-semibold text-info-content">
                <FiActivity className="text-info" />
                Car profile note
              </div>
              <div className="mt-1 text-sm text-base-content/70">
                This profile is optimized for route tracking and environmental
                telemetry. Image capture is disabled for vehicle sessions.
              </div>
            </div>
          </div>
        ...
        <>
          <div className="mt-5 rounded-[1.75rem] border border-base-300 bg-base-200/60 p-4">
            ...
            <div className="mt-4 space-y-3">
              <InfoStackRow label="Session name" value={activeMissionName} />
              <InfoStackRow
                label="Mission id"
              ...
              <InfoStackRow
                label="Sample rate"
                value={
                  activeProfile?.sample_hz != null
                    ? `${activeProfile.sample_hz} Hz`
                    : "—"
                }
                icon={FiActivity}
              />
