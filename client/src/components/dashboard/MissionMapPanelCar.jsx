...
export default function MissionMapPanelCar({
  ...
  async function handleSaveGpsNamedLocation(name) {
    ...
    try {
      ...
      setPickerErrorText(result?.error || "Drive session could not be started.");
    } finally {
      setPickerBusy(false);
    }
  }
  ...
  const liveTelemetry = deviceState?.gps?.last_good_fix
    ? {
        lat: deviceState.gps.last_good_fix.lat,
        lon: deviceState.gps.last_good_fix.lon,
        alt_m: deviceState.gps.last_good_fix.alt_m ?? null,
        satellites:
          deviceState.gps.last_good_fix.satellites ??
          deviceState?.gps?.satellites ??
          null,
        hdop:
          deviceState.gps.last_good_fix.hdop ??
          deviceState?.gps?.hdop ??
          null,
        ts_epoch: deviceState.gps.last_good_fix.ts_epoch ?? null,
      }
    : null;
        ...
        <div className="relative min-h-130 flex-1 bg-base-200 xl:min-h-0">
          ...
          <div className="absolute inset-0">
            <MissionDashboardMap
              ...
              missionRunning={missionRunning}
              liveMapEnabled={Boolean(deviceState?.gps?.last_good_fix)}
              liveConnected={Boolean(deviceState?.gps?.online)}
