...
function InfoRow({ label, value, mono = false }) {
  return (
    <div className="flex items-start justify-between gap-4 rounded-xl border border-white/5 bg-white/5 px-3 py-2.5">
      <span className="text-xs uppercase tracking-wide text-neutral-content/60">
        {label}
      </span>
      <span
        className={`text-right text-sm font-medium text-neutral-content ${mono ? "font-mono break-all" : ""}`}
      >
        {value}
      </span>
    </div>
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-neutral-content/60">
              Runtime state, connection details and GPS diagnostics
            </p>
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-2">
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
              ...
              <div className="space-y-2.5">
                <InfoRow
                  label="Current state"
                  value={toTitleCase(runtimeState)}
                />
                <InfoRow
                  label="Connection"
                  value={
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
              ...
              <div className="space-y-2.5">
                ...
                <InfoRow
                  label="Satellites"
                  value={hasCurrentFix ? (gps?.satellites ?? "—") : "—"}
                />
                <InfoRow
                  label="HDOP"
                  value={hasCurrentFix ? formatNumber(gps?.hdop) : "—"}
                />
              </div>
              ...
              {fix?.lat != null && fix?.lon != null ? (
                <div className="mt-3 rounded-xl border border-info/30 bg-info/10 p-3">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    ...
                    <InfoRow
                      label="Alt"
                      value={formatNumber(fix.alt_m, 1, " m")}
                    />
                  </div>
                </div>
              ) : (
              ...
              <div className="xl:col-span-2 rounded-2xl border border-warning/30 bg-warning/10 p-4">
                ...
                <div className="space-y-2">
                  {warnings.map((warning, index) => (
                    <div
                      key={index}
                      className="rounded-xl border border-warning/40 bg-warning/20 px-3 py-3 text-sm text-warning-content font-medium"
                    >
                      {warning}
                    </div>
                  ))}
